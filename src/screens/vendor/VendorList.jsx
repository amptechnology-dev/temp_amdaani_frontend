...
const VendorList = ({ isNavbar = true }) => {
  ...
  const fetchVendors = useCallback(async (page = 1, append = false) => {
    ...
    try {
      ...
      const {
        docs = [],
        hasNextPage: nextHasNextPage = false,
      ...
      if (mountedRef.current) {
        ...
        Toast.show({
          type: 'error',
          text1: 'Vendors',
          text2: err.message || 'Failed to load vendors',
        });
      }
    } finally {
  ...
  const onDueRefresh = useCallback(() => {
    ...
    fetchDueVendors().finally(() => {
      if (mountedRef.current) setDueRefreshing(false);
    });
  }, [fetchDueVendors]);
            ...
            onPress: async () => {
              ...
              try {
                ...
                Toast.show({
                  type: 'error',
                  text1: 'Delete failed',
                  text2: err?.message || 'Unable to delete vendor',
                });
              }
            },
          },
  ...
  const AllVendorsRoute = () => (
    <FlatList
      ...
      contentContainerStyle={[
        ...
        filteredVendors.length === 0 && styles.emptyListContainer,
      ]}
      refreshControl={
  ...
  const DueVendorsRoute = () => {
    ...
    return (
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={dueRefreshing} onRefresh={onDueRefresh} />
        }
  ...
  const handleNavigateToEdit = useCallback(
    vendor => navigation.navigate('AddVendor', { vendor }),
    [navigation],
  );

  const calculateTotalDue = vendorList => {
    return vendorList.reduce((total, vendor) => total + vendor.totalDue, 0);
  };
  ...
  const renderVendorDueCard = vendor => {
    ...
    return (
      <Card
        ...
        onPress={() =>
          navigation.navigate('VendorDueDetails', { id: vendor.id })
        }
        style={[
          styles.card,
            ...
            >
              ...
              <View style={styles.dueRight}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.dueBadgeText, { color: dueSeverity.color }]}
                  >
