...
const filterTransactionsByDate = (transactions, filter) => {
  if (filter === 'all') return transactions; // Show all by default
...
const Transactions = ({ isNavbar = true }) => {
  ...
  const fetchPage = useCallback(
    async (pageToLoad = 1, { reset = false } = {}) => {
      ...
      try {
        ...
        const res = await api.get(`/invoice?page=${pageToLoad}&limit=500`);
  ...
  const applyFilters = useCallback(() => {
    const dateFiltered =
      filterTransactionsByDate(allTransactions, activeFilter) || [];
  ...
  const renderTransactionCard = ({ item: t }) => {
    const displayName = getCustomerDisplayName(t);
    const displayMobile = getCustomerDisplayMobile(t);
    const showPhoneIcon = !t?.customerName?.trim();
    return (
      <Card
        style={styles.card}
        contentStyle={{
        ...
        mode="outlined"
        onPress={() =>
          navigation.navigate('InvoiceDetail', { invoiceId: t._id })
        }
      >
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text variant="labelSmall" style={styles.date}>
              {format(new Date(t.invoiceDate), 'dd MMM yyyy, hh:mm a')}
            </Text>
          </View>
  ...
  return (
    <>
      ...
      <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
        {isNavbar && <Navbar title="Invoices" />}
          ...
          <View style={{}}>
            <FlatList
              ...
              renderItem={({ item: f }) => (
                <Chip
                  mode={
                    activePaymentStatusFilter === f.value ? 'flat' : 'outlined'
                  }
      ...
      {!isNavbar && filteredTransactions.length > 0 && (
        <FAB
