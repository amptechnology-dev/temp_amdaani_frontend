...
import CustomerDetailSheet from '../../components/BottomSheet/CustomerDetailsSheet';
...
export default function NewSale() {
  ...
  const invoiceCalculations = useMemo(() => {
    ...
    const computedItems = (cartItems || []).map(item => {
      ...
      if (isGstInvoice) {
        ...
        const igstAmount = 0; // assuming no IGST for simplicity
    ...
    const roundOff = parseFloat((roundedTotal - grandTotalRaw).toFixed(2));
  ...
  }, [cartItems, isGstInvoice, discount]);

  React.useEffect(() => {
    const gt = Number(invoiceCalculations?.roundedTotal ?? 0);
    const disc = Math.round(Number(invoiceCalculations?.discountTotal));
    // Only auto-update if user hasn’t manually edited paid amount
    if (!hasUserEditedPaid.current) {
      setPaidAmount(gt - disc);
    }
    // Also auto-adjust if discount changes (but only if user didn’t override)
  }, [invoiceCalculations?.roundedTotal, invoiceCalculations?.discountTotal]);
  ...
  const handleAddItems = () => {
    ...
    const preparedCart = (cartItems || []).map((item, index) => ({
      ...item,
      _id: item._id || item.productId || `${item.name}-${index}`, // fallback for matching
  ...
  return (
    <>
                    ...
                    <WebView
                      ...
                      source={{
                        html: generateInvoiceHTML({
                          formValues: values,
                          // ✅ Always use computedItems for accurate GST, totals, etc.
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <TextInput
                          ...
                          onChangeText={txt => {
                            // Only allow changes if customer mobile is available
                            if (
                              !formikRef.current?.values?.contactNumber?.trim()
                            ) {
                              Toast.show({
                                type: 'error',
                                text1: 'Customer Mobile Required',
                                text2:
                            ...
                            const gt = Number(
                              invoiceCalculations?.roundedTotal ?? 0,
                            );
                            setPaidAmount(
                          ...
                          theme={{ roundness: 12 }}
                          contentStyle={{ fontSize: 13, paddingVertical: 0 }}
                        />
                      </View>
                      <View
                        style={{
      ...
      <CustomerDetailBottomSheet
        ref={customerSheetRef}
        customerData={{
          // Pull latest values from Formik at render time
