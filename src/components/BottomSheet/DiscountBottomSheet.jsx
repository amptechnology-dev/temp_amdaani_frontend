      ...
      >
        ...
        >
          ...
          <TextInput
            mode="outlined"
            label={
              localDiscount.type === 'flat' ? 'Amount (₹)' : 'Percentage (%)'
            }
            keyboardType="numeric"
            value={String(localDiscount.value || '')}
            onChangeText={text => {
              const cleaned = text.replace(/[^0-9.]/g, '');
              const valid = cleaned.replace(/^(\d*\.?\d{0,2}).*$/, '$1');

              // Cap percent at 100, flat has no upper cap
              if (localDiscount.type === 'percent') {
                const num = Number(valid);
                if (num > 100) return; // block input above 100
              }

              setLocalDiscount({ ...localDiscount, value: valid });
            }}
            style={styles.input}
            left={
              <TextInput.Icon
                icon={
                  localDiscount.type === 'flat' ? 'currency-inr' : 'percent'
                }
              />
            }
            theme={{ roundness: 12 }}
